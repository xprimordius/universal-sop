# PRIME SOP v2.2 — The Unconditional Foundation
...
**Updated:** March 14, 2026 — Session 6 (v2.2: Alan's full verbatim prompts embedded + detailed checklist built from them)
**Previous Version:** v2.0 (in-session), v1.1 backed up at `cache/backups/20260314_090000_v11_session6_prerebuild/PRIME_SOP.md`
...
- [ ] C1. Stage 1 = Clarity & Coherence — relay back as numbered checklist (verbatim requirement #1)
- [ ] C2. Wait for human confirmation (💠) before proceeding (verbatim requirement #1)
- [ ] C3. Reference Stage 1 checklist at end of output with status per item (verbatim requirement #1)
- [ ] C4. Conclusion checklist of what's done vs. prompt at end of every output (verbatim requirement #8)
- [ ] C5. Timeline position visual at end of every output (verbatim requirement #8)
- [ ] C6. Visuals to accelerate understanding — icons, tables, timelines (verbatim requirement #8)

### ACCURACY & SOURCES
- [ ] A1. Every output factual and indisputable upon clerical checks and audits (verbatim requirement #2)
- [ ] A2. Verbatim quotes with human-verifiable file paths for fact-checking (verbatim requirement #2)
- [ ] A3. Source validity / accuracy / coherence verified — practical intuitive reasoning any sound mind would agree with (verbatim requirement #2)
- [ ] A4. Builder fact-checks own sources before presenting (verbatim requirement #2)

### QUALITY STANDARD
- [ ] Q1. Accuracy, precision, meticulousness, and due diligence at PRISTINE level (verbatim requirement #3)
- [ ] Q2. Perfected. Refined. Remastered. Forged to perfection. (verbatim requirement #3)
- [ ] Q3. World-class standard: brain surgeons, engineers, researchers, architects level (verbatim requirement #5)
- [ ] Q4. Everything is a SEED — manifested extension and evolution of these qualities (verbatim requirement #3)

### AGENTS & VERIFICATION
- [ ] V1. Agents deployed at verify sources, self-check, validation, verify, document stages (verbatim requirement #4)
- [ ] V2. Final coherence check at end with the Stage 1 checklist (verbatim requirement #4)
- [ ] V3. Proactive problem solving with reserved tokens — troubleshoot BEFORE final output (verbatim requirement #12)
- [ ] V4. Document, record, file everything in detail with timestamps (verbatim requirement #12)

### DOCUMENTATION & CACHING
- [ ] D1. Every new SOP announced clearly — tied to physical document referenceable by both parties (verbatim requirement #9)
- [ ] D2. Checkpoint labels: cache saved + file name + location (verbatim requirement #10)
- [ ] D3. Backup and label previous working versions before overwriting (verbatim requirement #11)
- [ ] D4. Checkpoint cost breakdown: what files, why, how many tokens required + used (verbatim requirement #11)
- [ ] D5. Detailed prompts like this cached verbatim as "quality core standards" (verbatim requirement #6)

### CONTINUOUS IMPROVEMENT
- [ ] I1. Routine review: identify bottlenecks, opportunities for growth, improvement, enhancement (verbatim requirement #7)
- [ ] I2. This is its own supplementary operation in detail — not optional (verbatim requirement #7)

**Total: 24 checklist items derived directly from Alan's verbatim prompts.**
**Each item traces to a specific verbatim quote above.**
...
10. Log findings briefly: "Pre-send check: [X] items checked, [Y] gaps found, [Z] fixed"
11. Document with timestamp for future reference
...
9. **Timeline Position** (end of every output): Visual of where we are in the total process
```
📍 TIMELINE
═══✅═══✅═══✅═══🔄═══❌═══❌═══❌
 1     2     3     4     5     6     7
...
## KNOWN FAILURE POINTS & SOLUTIONS (Lessons from Sessions 1-5)

| # | Failure | Root Cause | Solution | Stage That Catches It |
|---|---------|-----------|----------|----------------------|
| 1 | Token tracking missing | No enforcement mechanism | Prime Checklist item #5-7 | Stage 7 (Self-Check) |
| 2 | Agent never ran on deliverable | Wrong prompt pattern | "Read files first" in agent prompt | Stage 8 (Validate) |
| 3 | Step headers missing | Forgot under content pressure | Checklist item #8 | Stage 7 |
| 4 | POVP/PFV skipped | Focus shifted to content | Checklist items #10-11 | Stage 7 |
| 5 | Backup before edit skipped | Rushed to deliver | Checklist item #16, Stage 10 protocol | Stage 10 (Document) |
| 6 | User prompt missed/partial | Didn't re-read original prompt | Stage 7.5 proactive check | Stage 7.5 |
| 7 | Promise without delivery | Said "will do" then didn't | Stage 9 verification | Stage 9 (Verify) |
| 8 | File saves unlabeled | No save confirmation protocol | Save Confirmation Block | Stage 10 |
| 9 | Worked past YELLOW/RED | Prioritized content over checkpoint | Corrected thresholds + checkpoint reserve | Stage 3 (Measure) |
| 10 | Stage 1 relay-back skipped | Treated Prime SOP as document not checklist | Execute, don't just read | Stage 1 |
| 11 | Repeated user prompts needed | Rule written but not enforced | Prime SOP = enforcement mechanism | ALL stages |
...
*Prime SOP v2.2 — The foundation that cannot be broken. Everything else is built on this.*
