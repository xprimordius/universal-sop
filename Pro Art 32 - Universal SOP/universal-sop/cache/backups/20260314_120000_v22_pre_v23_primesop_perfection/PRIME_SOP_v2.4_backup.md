# PRIME SOP v2.4 — The Unconditional Foundation
...
**Updated:** March 14, 2026 — Session 7 (v2.4: Cannot-miss priorities, unconditional domains, procedure summary, suboutput labeling, dependency gate, breaking point scanner added)
**Previous Version:** v2.3 backed up at `cache/backups/20260314_120000_v22_pre_v23_primesop_perfection/PRIME_SOP_v2.3_backup.md`
...
**WHY:** Every recurring failure in this project traces to one root cause: rules were written but not enforced in execution. Token tracking was documented but forgotten. Backups were required but skipped. Checklists were built but not used. The Prime SOP exists to make forgetting IMPOSSIBLE.

**WHAT:** This is not a reference document. It is a LIVING operational checklist — 12 stages that EVERY output moves through, unconditionally. It is the surgeon's pre-operative protocol. The pilot's pre-flight sequence. The engineer's load calculation before the first beam is placed.

**HOW:** The Prime SOP is loaded FIRST at session start via CLAUDE.md. It is executed — not just read — on every single message. Stage 1 (Understanding) happens before work begins. Stages 7-9 (Self-Check, Validate, Verify) happen before delivery. Stage 11 (Communicate) structures every output. Stage 12 (Confirm) closes every cycle.

---

## ⚡ CANNOT-MISS PRIORITIES (Quick Reference — Read First Every Session)

| # | Priority | One-Liner | If Missed |
|:-:|----------|-----------|-----------|
...
| P9 | **Never break the foundation** | Prime SOP is unconditional. All 12 stages. Every message. | System collapses. |
...
|:-:|-------|-------------------------------|--------|
| C1 | Stage 1 relay-back as clean table checklist | Output contains `✅ UNDERSTANDING CHECK` table with numbered items + ☐ boxes | Prompt #7 |
| C2 | Wait for 💠 confirmation before proceeding | No execution begins until Alan responds with confirmation | Prompt #7 |
| C3 | Stage 1 checklist referenced at end with status | Output ends with `📋 CONCLUSION` table showing ✅/🔄/❌ per item from Stage 1 | Prompt #7 |
| C4 | Conclusion checklist of done vs. prompt | `📋 CONCLUSION` block present at end of every output | Prompt #9 |
| C5 | Timeline with NAMED steps | `📍 Position` visual present with step names (not just numbers) | Prompt #9, #18 |
| C6 | Visuals: icons, tables, timelines used | No walls of text — data in tables, status in icons, position in timeline | Prompt #3 |

### ACCURACY & SOURCES

| # | Check | How to Verify (PASS Criteria) | Source |
|:-:|-------|-------------------------------|--------|
| A1 | Every claim sourced — nothing assumed | Each factual claim has `← [file] @ [path]` or verbatim quote | Prompt #7 |
| A2 | Verbatim quotes with file paths | Quoted text in `" "` with file path Alan can open and verify | Prompt #7 |
| A3 | Source validity confirmed | Sources are current version, not superseded, no cross-cache contradictions | Prompt #7 |
| A4 | Builder fact-checks before presenting | Files READ (not recalled from memory) — Read tool used, not assumption | Prompt #13 |

### QUALITY STANDARD

| # | Check | How to Verify (PASS Criteria) | Source |
|:-:|-------|-------------------------------|--------|
| Q1 | Pristine accuracy + precision | Zero errors upon clerical audit. Every detail correct. Nothing partial. | Prompt #7 |
| Q2 | Perfected, refined, forged | Output is not a rough draft — it's polished, structured, final-quality | Prompt #7 |
| Q3 | World-class professional standard | Would a surgeon / engineer / architect approve this level of rigor? | Prompt #7 |
| Q4 | Seed quality — extension-ready | Output can be built upon without rework. Foundation, not throwaway. | Prompt #7 |

### AGENTS & VERIFICATION

| # | Check | How to Verify (PASS Criteria) | Source |
|:-:|-------|-------------------------------|--------|
| V1 | Agents deployed at correct stages | Jenny/Karen on framework deliverables. QA on every output. QE at session end. | Prompt #5 |
| V2 | Final coherence check with Stage 1 list | Last section of output = Stage 1 items with completion status | Prompt #7 |
| V3 | Proactive error check with reserved tokens | Pre-send check log line present: "Pre-send check: X items, Y gaps, Z fixed" | Prompt #16 |
| V4 | Everything documented with timestamps | File saves have Save Confirmation Block. Changes logged with dates. | Prompt #16 |

### DOCUMENTATION & CACHING

| # | Check | How to Verify (PASS Criteria) | Source |
|:-:|-------|-------------------------------|--------|
| D1 | New SOPs announced clearly | Chat contains: `📄 [NAME] v[#] saved to [path]` for every new/updated file | Prompt #5 |
| D2 | Checkpoint labels: file + name + location | Checkpoint output lists every file saved with name, path, version | Prompt #8 |
| D3 | Backup before overwrite — named, rollback-safe | Backup file exists at `cache/backups/` with version name BEFORE edit happens | Prompt #10, #19 |
| D4 | Checkpoint token breakdown | Table showing: operation, est. tokens, actual tokens | Prompt #11 |
| D5 | Verbatim prompts cached | `USER_PROMPTS_VERBATIM.md` contains all prompts word-for-word | Prompt #15, #19 |

### CONTINUOUS IMPROVEMENT

| # | Check | How to Verify (PASS Criteria) | Source |
|:-:|-------|-------------------------------|--------|
| I1 | Routine review at session end | QE agent runs. Bottlenecks identified. Improvements proposed. | Prompt #7 |
| I2 | Improvement is its own operation | Separate, documented step — not tacked on as afterthought | Prompt #7 |

**Total: 24 checklist items. Each has PASS criteria + source prompt reference.**
**Verbatim source prompts: `cache/USER_PROMPTS_VERBATIM.md`**
...
## MY PROCEDURE — IN 12 HEADLINES

Here is what I do and why, in plain human language:

| # | What I Do | Why |
|:-:|-----------|-----|
| 1 | **Read your prompt and relay it back as a checklist** | So I never misunderstand what you're asking |
| 2 | **Check alignment against your prompts and SOPs** | So I never do work that doesn't serve the project |
| 3 | **Estimate token cost before starting** | So we never run out of budget mid-work |
| 4 | **Plan the work: what files, what agents, what risks** | So I never improvise or wing it |
| 5 | **Verify every source by reading the actual file** | So nothing I say is assumed or hallucinated |
| 6 | **Draft the output with visual structure** | So you can scan it in seconds, not minutes |
| 7 | **Self-check against 18-item foundation checklist** | So I catch my own errors before anyone else |
| 7.5 | **Proactive error scan against your original prompt** | So the #1 failure (missing requirements) never ships |
| 8 | **Agents validate independently (Jenny + Karen)** | So my own bias doesn't hide errors |
| 9 | **Verify completeness — 100% or state what's missing** | So "done" actually means done |
| 10 | **Document: backup, save, label, announce** | So nothing is lost between sessions |
| 11 | **Communicate: debrief, position, icons, timeline, conclusion** | So you always know where we are |
| 12 | **Wait for your confirmation, then improve the system** | So nothing ships without your approval |
...
2. Calculate: will this push cumulative past YELLOW (90K)?
3. If approaching YELLOW → alert Alan. Plan remaining deliverables.
4. If approaching RED (110K) → STOP content work. Execute checkpoint.
...
**WHO HELPS:** None — builder measures.

**TOKEN THRESHOLDS:**
| Status | Icon | Range | Checkpoint Reserve | Action |
|--------|------|-------|-------------------|--------|
| GREEN | 🌱 | <90K | 10K reserved | Full operations. Auto-continue. |
| YELLOW | ⚠️ | 90-110K | 10K reserved | Alert Alan. Plan final deliverables. |
| RED | ❗ | 110-130K | Executing checkpoint | STOP content. Checkpoint NOW. |
| CRITICAL | ⛔ | >130K | Unsafe | Emergency cache dump. STOP. |

**CHECKPOINT COST BREAKDOWN (~10K tokens):**
...
**WHY:** Measure twice, cut once. Before anyone else checks this, YOU check it. The surgeon counts instruments before closing. The engineer reviews calculations before submitting. The record keeper audits entries before filing. This is the internal quality gate.
...
**WHO HELPS:** None — builder communicates.
...
| **Quality Assurance (QA)** | Consistency watchdog | 7, 7.5, 11 | Micro-level: Is token tracking present? Is icon guide showing? Are save confirmations present? Is Stage 1 checklist referenced at conclusion? Catches the RECURRING failures that slip through repeatedly. Runs on EVERY output, not just framework deliverables. |
...
**PASS/FAIL:** All 10 items must be ✅. Any ❌ = fix before sending.
...
| 9 | Worked past YELLOW/RED | Prioritized content over checkpoint | Thresholds: YELLOW 90K, RED 110K. 10K checkpoint reserve. | Session stops at YELLOW, checkpoints at RED = PASS | 3 |
...
- It is NOT a suggestion. It is the floor. The minimum. The non-negotiable standard.

---

## WHAT THIS SOP IS

It is the foundation that cannot be broken. Everything else — every operational SOP, every project SOP, every module, every lesson — is a manifested extension and evolution of the traits, foundations, core, and qualities seeded here.

Pristine. Excellent. Perfected. Refined. Remastered. Forged to perfection.

---

*Prime SOP v2.4 — The foundation that cannot be broken. Everything else is built on this.*
