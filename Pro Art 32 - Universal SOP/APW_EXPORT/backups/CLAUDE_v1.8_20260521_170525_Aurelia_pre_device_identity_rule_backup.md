...
3. **Log every backup** in `cache/BACKUP_LOG.md` in the same operation.
4. **Applies to ALL files**, not just `cache/*`.

Full rule + table + filename conventions: see `SESSION_START.md` § "UNIVERSAL BACKUP & PRESERVATION RULE". **This supersedes and broadens the legacy `CACHE MODIFICATION PROTOCOL (D32)` below** (cache-only → all files). Mechanical steps identical.
