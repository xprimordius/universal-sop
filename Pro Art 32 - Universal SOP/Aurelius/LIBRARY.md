...
| **GitHub** | Cloud storage | Backs up the vault to two remote accounts | github.com/xprimordius + github.com/Awake999 |
...
Claude session JSONL (in ~/.claude/projects/)
      ...
      All three written to the vault folder
              ...
              git commit + git push
                        │
                        ▼
       github.com/xprimordius/aurelius-vault  ← cloud backup
       (also stays on github.com/Awake999 as frozen snapshot)
```

**Result**: every chat is in 4 places: vault on disk, vault in local git, vault on GitHub-xprimordius, vault on GitHub-Awake999.
...
| **Risk of losing data** | High (chat can drift, compact, clear) | Zero (4-way redundancy) |
...
└── 📁 Reference\             ← Long-form reference docs (the encyclopedia shelf)
    ...
    ├── Agent-Roster.md                ← Catalog of all 9 specialist agents
...
| **GitHub (Awake999, frozen)** | Same | Same |

**You are not running out of space.** The vault is tiny. Two full backups exist. We can run for years before space matters.
...
Because:
- Code: in 2 GitHub repos
- Vault notes (140+ files): in 2 GitHub repos
