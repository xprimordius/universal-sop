...
## Devices in this tailnet (live registry)

| Device | Role | Status | Identity | Notes |
|---|---|---|---|---|
| **Aurelion** (Strix Halo) | Primary brain + workstation | ✅ active | `Alan @ Aurelion` | Runs LM Studio + daily routine |
| **{Future Mac}** | Mobile work | 🔄 planned | `Alan @ Mac-name` | Vault read/write, voice via cloud_relay |
| **{Future iPhone}** | On-the-go | 🔄 planned | n/a (no git) | Obsidian Mobile + browser → cloud_relay |
...
6. **Two GitHub mirrors** (xprimordius + Awake999) = 2 cloud copies
7. **Local .git on each machine** = N device copies of full history

Total redundancy: **N devices + 2 cloud accounts = N+2 places where the data lives**. Losing all of them requires simultaneous failure of every machine you own AND both GitHub accounts. Effectively zero risk.
