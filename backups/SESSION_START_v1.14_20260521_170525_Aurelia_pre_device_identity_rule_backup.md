...
| 3 | **Timestamped filename** | Top-level `backups/`: `<original>_v<ver>_<YYYYMMDD>_<HHMMSS>_<reason>_backup.<ext>`. `cache/backups/`: `<YYYYMMDD>_<HHMMSS>_v<ver>_<reason>/` (subdirectory style for cache fan-out). Full `HHMMSS` preferred for unambiguity; date-only acceptable for legacy precedent. |
| 4 | **Log every backup** | In `cache/BACKUP_LOG.md` in the same operation as the edit, with: `# \| date \| time \| file \| version \| reason \| backup path`. |
...
**Origin:** 2026-05-21 — during Day-1 setup on a new device, AI appended a row to `cache/BACKUP_LOG.md` without a pre-edit backup file. Alan codified the rule via `/remember` to make the discipline cross-device. Remediation backups recorded in `cache/BACKUP_LOG.md` rows #127–#128. Rule itself added in rows #129–#131.
