...
## 📋 RULE 6 — OUTPUT SELF-AUDIT (NEW 2026-05-26 per user "fix this to perfect it")

**Why this rule exists:** Through 24 commits on aurelion (2026-05-25 → 2026-05-26), repo-side quintessence reached 9.9/10. But the SOP itself was NOT being followed in Claude's own outputs — no 🎯 MODEL line, no STEP header, no UC table, no Sync Status, no RAE block. The same brain that authored the rules was bypassing them. User 2026-05-26 02:00 CDT called this out as the #1 objective: *"perfect pristine quintessential SOP that does not forget, does not fabricate, maintains 100% accuracy and fidelity, does what it is supposed to in full, verifies via a checklist, holds itself accountable... just following the SOP itself it has problems with. fix this to perfect it"*

**This rule is the structural answer.** Per Aurelius doctrine (LEARNINGS Section 8): *convention → code*. Don't ask Claude to remember to follow the SOP — make it mechanical.

### The mandatory pre-ship sequence (every OUTPUT-STANDARD / OUTPUT-COMPLEX)

```
1. Write draft to /tmp/output_draft.md   (or any path; you keep the response in context)
2. bash scripts/validator_output.sh /tmp/output_draft.md
3. If verdict = BLOCK (≥2 FAIL on mandatory items): fix the draft, re-validate.
4. If verdict = ALLOW (≤1 FAIL, all PASS preferred): proceed to step 5.
5. bash scripts/log_output.sh "<verbatim user prompt>" /tmp/output_draft.md <validator-score>
6. Send the response.
```

### What the validator checks (7 probes — all mechanical, no behavioral judgment)

| Probe | Detects | Source SubSOP |
|-------|---------|---------------|
| **P4 MES** | `🎯 MODEL: <tier> · <variant>` in first 10 lines | SP.21 MES (literal first content) |
| **STEP header** | `STEP: ... / 🎯 MACRO: ... / 📌 MICRO: ...` all present | MTL RULE 1 |
| **P2 IAC** | UC table with `\| C.N \|` clause rows OR verbatim quotes (trivial carve-out) | SP.6 IAC (doctoral precision) |
| **P3 SSC** | `📡 Sync Status` table OR `📦 BACKUP STATUS` block | SP.18 SSC v1.7.1 |
| **P5 RAE** | `🎯 RECOMMENDATIONS` block + autonomous-mode marker | SP.24 RAE v2 |
| **Pulse Check** | Visible `Pulse Check` block with `Score: N/5` line | EN.4 / MTL RULE 3 |
| **P1 Honest numbers** | No `exactly N` / `precisely N` without grounding | SP.5 FSP |

### What `scripts/log_output.sh` writes

| Surface | Content | Why |
|---------|---------|-----|
| `cache/OUTPUT_LOG.md` | One-line row per output: date / time / device / commit / prompt-chars / response-chars / validator-score / archive-path | "shows exactly what it is doing" — searchable index |
| `cache/prompt_archive/<ts>_<device>.md` | Verbatim user prompt + reference to response + validator score | "what the user prompts are" — never lose a prompt |

### Adoption + skip rules

| Output tier | RULE 6 status |
|---|---|
| **OUTPUT-COMPLEX** | MANDATORY — validator + log_output both run |
| **OUTPUT-STANDARD** | MANDATORY — validator + log_output both run |
| **OUTPUT-QUICK** (single fact, "yes" confirmation, conversational reply) | OPTIONAL — log_output recommended, validator skipped |
| **CONVERSATIONAL** (clarifying question, status update) | OPTIONAL — log_output only |

### Anti-patterns this rule prevents

- ❌ Shipping a long content-heavy response without 🎯 MODEL (caught by validator P4)
- ❌ Forgetting STEP header on a major work output (caught by validator STEP probe)
- ❌ Compressing UC away when user spec was multi-clause (caught by validator P2)
- ❌ "All looks good" with no Sync Status (caught by validator P3)
- ❌ Ending with "Reply X or Y or Z" (caught implicitly — RAE-autonomous probe expects auto-execute marker)

### What this rule does NOT solve

- Cannot enforce IF Claude actually invokes the validator (still requires Claude discipline)
- A future PreResponse Claude-Code hook would close this — `.claude/settings.local.json` would invoke validator on every Stop event and block stop on FAIL. Until that ships, RULE 6 is convention + script-assisted, not fully mechanical at the output boundary.
- F.59-style remote rejection is unrelated (commit-message level, not output level)
