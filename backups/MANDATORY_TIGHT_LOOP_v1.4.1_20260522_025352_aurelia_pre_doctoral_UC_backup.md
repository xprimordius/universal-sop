...
| 10 | **SP.21 MES** ⭐ NEW 2026-05-22 (F.25 / F.26 correction) | Model Effort Selector | Declare model variant + effort tier at the VERY TOP of every output. Two orthogonal axes: **Model variant** (Opus 4.7 · Opus 4.7 1M · Opus 4.6 legacy · Sonnet 4.6) AND **Effort tier** (max · very high · medium · low). One-sentence real-world reason. Conservative bias (higher effort > lower). Sonnet 4.6 only with absolute certainty (template-fill, zero ambiguity). Mid-output adjustment allowed. End-of-output retrospective (MES Retrospective: true best setting + calibration). Format: `🎯 MODEL: <variant> · <tier>` line at output top BEFORE STEP header. |
