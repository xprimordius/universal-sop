...
if [ -z "$TARGET_FILE" ]; then
  ...
  if [ -z "$STAGED_MODIFIED" ]; then
    ...
    STAGED_COUNT=$(printf "%s\n" "$STAGED_MODIFIED" | wc -l | tr -d ' ')
  fi

  if [ "$STAGED_COUNT" -gt 0 ]; then
    # Count NEW BACKUP_LOG rows in this diff (added lines starting with |NN|)
    NEW_BACKUP_ROWS=$(git diff --cached cache/BACKUP_LOG.md 2>/dev/null | grep -cE "^\+\|[[:space:]]*[0-9]+[[:space:]]*\|" 2>/dev/null || true)
    NEW_BACKUP_ROWS=${NEW_BACKUP_ROWS:-0}
    # Sanitize to single integer (grep -c can return non-numeric on edge cases)
    NEW_BACKUP_ROWS=$(printf "%s" "$NEW_BACKUP_ROWS" | head -1 | tr -d ' ')
    [ -z "$NEW_BACKUP_ROWS" ] && NEW_BACKUP_ROWS=0
