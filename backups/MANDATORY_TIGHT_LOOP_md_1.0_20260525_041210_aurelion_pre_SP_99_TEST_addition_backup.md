...
A future `validator.sh VL.12 PROVE-IT-presence` check will grep commit subjects matching `OP-LEVEL|F\.[0-9]+|Sprint [0-9]+ COMPLETE` and verify the body contains items 1-9 by number. Until shipped, this is convention-enforced (writer of the commit is responsible).
